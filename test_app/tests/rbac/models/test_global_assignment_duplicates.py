...
class TestGlobalUserAssignmentConstraint:
    ...
    def test_constraint_does_not_affect_object_assignments(self, rando, inventory, inv_rd):
        inv_rd.give_permission(rando, inventory)
        assert RoleUserAssignment.objects.filter(user=rando, role_definition=inv_rd, object_role__isnull=False).count() == 1
